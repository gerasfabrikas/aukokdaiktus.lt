...
<?php
	foreach(listData('cats', 'cat_type = 4 AND cat_level = 0') as $item) :
		echo '<optgroup label="'.$item['cat_name'].'">';
		echo '<option '.((isset($_GET['fc']) and $_GET['fc'] == $item['cat_id']) ? 'selected="selected"' : '').' value = "'.$item['cat_id'].'">'.$item['cat_name'].' (visi)</option>';
		foreach(listData('cats', 'cat_type = 4 AND cat_level = 1 AND cat_parent = '.$item['cat_id']) as $item2) :
			echo '<option '.((isset($_GET['fc']) and $_GET['fc'] == $item2['cat_id']) ? 'selected="selected"' : '').' value = "'.$item2['cat_id'].'">'.$item2['cat_name'].'</option>';
		endforeach;
		echo '</optgroup>';
	endforeach;
?>
...
<!-- SIDEBAR -->
	...
	<a href="http://www.aukoklaika.lt/" target="_blank"><div class="sidehead brothersite">&nbsp;</div></a>
